...
use crate::systems::state::{GameStage, PauseState, Session, StartupSequence};
    ...
    let _ = canvas.with_texture_canvas(&mut backbuffer.0, |canvas| {
        ...
        if matches!(session.stage(), GameStage::GameOver) {
            ...
            if let Err(e) = text.render_with_color(canvas, &mut atlas, t, pos, Color::RED) {
                tracing::error!("GAME OVER text render failed: {e}");
            }
        }
        ...
        if matches!(
            ...
            if matches!(session.stage(), GameStage::Starting(StartupSequence::TextOnly { .. })) {
                ...
                if let Err(e) = text.render_with_color(canvas, &mut atlas, t, pos, Color::CYAN) {
                    tracing::error!("PLAYER ONE text render failed: {e}");
                }
            }
        }
        ...
        if pause_state.active() {
            ...
            if let Err(e) = paused.render_with_color(canvas, &mut atlas, t, pos, Color::YELLOW) {
                tracing::error!("PAUSED text render failed: {e}");
            }
        }
    });
}
